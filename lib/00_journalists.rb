...
def num(array)
    return array.count {|ele| ele =~ /\d/ }
end

def aude(array)
    return array.count {|ele| ele.downcase.include? "aude" }
end

def maj_start(array)
    return array.count {|ele| ele[1,1].match(/[A-Z]/)}
end

def maj_all(array)
    return array.count {|ele| ele =~ /[[:upper:]]/}
end

def under_count(array)
    n=0
    array.each do |ele|
        n+=ele.count"_"
    end
...
def menu
    ...
    9. Quelle est la position dans l'array de la personne suivante...\n"
    print "> "
    ...
    when 3
        puts aude(journa)
    when 4
        puts maj_start(journa)
    when 5
        puts maj_all(journa)
    when 6
        puts under_count(journa)
    when 7
        puts tri(journa)
    when 8
